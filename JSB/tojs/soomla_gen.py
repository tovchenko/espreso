...
def main():
    ...
    src_root = os.path.abspath(os.path.join(project_root, '..', 'External/cocos2dx-store/Soomla'))
    ...
    try:
        ...
        tojs_root = '%s/tojs' % project_root
        output_dir = '%s/js-bindings/bindings/auto' % project_root

        cmd_args = {'soomla.ini' : ('soomla', 'jsb_soomla_auto')}
        target = 'spidermonkey'
