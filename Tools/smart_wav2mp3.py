	...
	for filename in os.listdir(fullSrcPath):
		...
		if os.path.isdir(srcFilePath):
			...
			if ext == 'wav':
				...
				dstFilePath = os.path.join(dstPath, filename.replace('.wav', '.mp3'));
				...
				print 'Convert ' + filename + ' to mp3 file.'
				ffmpeg = os.path.join(os.path.dirname(os.path.realpath(__file__)), './ffmpeg/ffmpeg')
				#subprocess.call([ffmpeg, '-i', srcFilePath, '-y', '-loglevel', 'panic', '-f', 'mp3', '-vn', '-acodec', 'libvorbis', '-aq', '60', dstFilePath])
				#subprocess.call([ffmpeg, '-i', srcFilePath, '-y', '-loglevel', 'panic', '-f', 'mp3', '-vn', '-aq', '60', dstFilePath])
				subprocess.call([ffmpeg, '-i', srcFilePath, '-y', '-loglevel', 'panic', '-f', 'mp3', '-vn', dstFilePath])
		else:
